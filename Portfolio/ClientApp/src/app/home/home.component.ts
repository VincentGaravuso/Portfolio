...
export class HomeComponent implements OnInit {
  ...
  populateCards() {
    ...
    for (var i = 0; i < this.userRepos.length; i++) {
      ...
      cardTitle.textContent = this.userRepos[i].name;

      var cardDescription = document.createElement('p');
      cardDescription.className = 'card-text';
      cardDescription.textContent = this.userRepos[i].description;

      var cardFooter = document.createElement('p');
      cardFooter.className = 'card-text';

      var calSVG = document.createElement('img');
      calSVG.src = '../../assets/github-logo.svg';
      calSVG.setAttribute('style', 'width:30px;height:30px;filter:brightness(0);');

      var lastUpdated = document.createElement('small');
      lastUpdated.className = 'text-muted';
      lastUpdated.textContent = 'Last Updated: ' + this.datepipe.transform(this.userRepos[i].updated_at, 'MM/dd/yy').toString();

      var repoLink = document.createElement('a');
      repoLink.className = 'card-link stretched-link';
      repoLink.href = this.userRepos[i].html_url;
      repoLink.target = '_blank';
      repoLink.setAttribute('style', 'float:right;');
      repoLink.appendChild(calSVG);
      var languageDiv = document.createElement('div');

      var languageBar = document.createElement('small');
      languageBar.className = 'card-text';
      languageBar.textContent = this.userRepos[i].language;

      var languageDot = document.createElement('div');
      var color = this.languageColors[this.userRepos[i].language].color;
      languageDot.setAttribute('style', `background: ${color}; width:10px;height:10px;border-radius:50%; display:inline-block; margin-right:2px; vertical-align:middle;`);

      languageDiv.appendChild(languageDot);
      languageDiv.appendChild(languageBar);
      cardFooter.appendChild(lastUpdated);
      cardFooter.appendChild(repoLink);
      cardBody.appendChild(cardTitle);
      cardBody.appendChild(cardDescription);
      cardBody.appendChild(languageDiv);
